...
COMPOUND TRIGGER
	...
	ELSE
		...
		UPDATE estadio
		SET equipo = (SELECT TREAT(REF(p) AS REF equipo_t) FROM equipo p WHERE p.nombre = equipos.nombre) 
		WHERE nombre = equipos.nombre;
...
COMPOUND TRIGGER
	...
	ELSE
		...
		UPDATE equipo
		SET estadio = (SELECT TREAT(REF(p) AS REF estadio_t) FROM estadio p WHERE p.nombre = estadios.nombre) 
		WHERE nombre = estadios.nombre;
